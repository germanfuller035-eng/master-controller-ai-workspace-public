#!/usr/bin/env node
// tools/customer_success_os/tests/security.test.mjs
// Phase 50: Security scan over Customer Success OS. No secrets, no send, no publish, no production, no real customer.
...
const GEN = path.resolve(ROOT, '../../_generated/customer_success_os');
...
let secretHits = 0, sendHits = 0, pubHits = 0, prodHits = 0, identityHits = 0, contactHits = 0, sendAllowedTrue = 0;
for (const f of files) {
  const txt = readFileSync(f, 'utf8');
  const rel = path.relative(ROOT, f);
  for (const s of SECRET_RX) if (s.re.test(txt)) { secretHits++; console.log(`  secret-like (${s.name}) in ${rel}`); }
  if (!/tests[\\/]|validators\.mjs$|incidents\.mjs$|support\.mjs$|permissions\.mjs$/.test(rel)) {
    if (/(nodemailer|createTransport|\.sendMail\s*\(|bot\.sendMessage\s*\()/.test(txt)) { sendHits++; console.log(`  send in ${rel}`); }
    if (/(publishSite|publishKb|publishCase|goLive)\s*\(/.test(txt)) { pubHits++; console.log(`  publish in ${rel}`); }
    if (/(ssh2|195\.96\.132\.82|exec.*systemctl)/.test(txt)) { prodHits++; console.log(`  prod access in ${rel}`); }
    if (/setLeadStatus|writeCanonicalIdentity/.test(txt)) { identityHits++; console.log(`  canonical identity write in ${rel}`); }
  }
  // Fixtures must not carry real identity (name/email/phone).
  if (/fixtures|samples|generated/.test(rel)) {
    if (REAL_EMAIL_RX.test(txt)) { contactHits++; console.log(`  real email in ${rel}`); }
    if (/customers\.json$/.test(rel) && PHONE_RX.test(txt)) { contactHits++; console.log(`  phone in ${rel}`); }
  }
...
ok('no canonical identity write', identityHits === 0, `hits=${identityHits}`);
ok('no real contacts in fixtures', contactHits === 0, `hits=${contactHits}`);
