...
const FIRST_TOUCH_FORBIDDEN_RX = /\bROI\b|\bconversion\b|\btraffic\b|\bgrowth\b|client-volume|конверс|трафик|рост продаж|увеличим продажи|\d[\d\s]*₽|\d+\s*руб|оплат|договор|выставить\s+сч[её]т|нашли у вас проблему|точно есть проблема|вы теряете|\d+\s*%/i;
...
export function validateMessageArchitecture(ma) {
  const errors = [];
  if (ma.rules?.send !== false) errors.push('message_architecture: send must be false');
  for (const [id, text] of Object.entries(ma.variants || {})) {
    const body = String(text || '');
    if (!/меня зовут дмитрий/i.test(body)) errors.push(`${id}: missing human intro`);
    if (!/(первого касания|первое касание)/i.test(body)) errors.push(`${id}: missing first-touch context`);
    if (!/посмотрел/i.test(body)) errors.push(`${id}: missing visible site context`);
    if (!/(не утверждаю|не делаю вывод)/i.test(body)) errors.push(`${id}: missing certainty caveat`);
    if (!/3[-–]5/.test(body)) errors.push(`${id}: missing 3-5 point mini-review`);
    if (!/кому у вас удобнее передать/i.test(body)) errors.push(`${id}: missing low-pressure transfer CTA`);
    if (FIRST_TOUCH_FORBIDDEN_RX.test(body)) errors.push(`${id}: contains forbidden first-touch claim or pressure`);
  }
  return errors;
}
