#!/usr/bin/env node
// tools/security_os/apply_manifest.mjs — proposed-apply manifest + registry proposal (MP49). Read-only.
...
for (const f of walk(PROPOSED)) {
  const m = meta(f);
  if (m.project !== 'security-control-plane') continue;
...
const manifest = { schema: 'security_os.apply_manifest.v1', generated_ts: TS, vault: VAULT, apply_command: 'AI_HQ_APPLY_OK=1 node tools/ai_hq/apply_canonical.mjs --apply  (owner only, post-review)', note: 'Proposed security docs NOT applied. Production freeze in effect.', canonical_docs_applied: 0, total: rows.length, rows };
...
const registry = { schema: 'security_os.registry_proposal.v1', generated_ts: TS, note: 'Proposed Project Registry addition; NOT applied.', target: '00_MASTER_CONTEXT/PROJECT_REGISTRY.md (owner-applied)', security_control_plane_project_entry: { project_id: 'security-control-plane', name: 'Security / Privacy / Compliance Control Plane', status: 'ACTIVE_DEVELOPMENT', priority: 'P1_SECURITY', source_of_truth: 'tools/security_os + docs_canonical_proposed/16_security/', note: 'Security governance + evidence-based control catalog. Static/offline; never stores secret values, uses/rotates credentials, scans live hosts, deletes, sends, mutates production, or asserts legal compliance.' } };
writeFileSync(path.join(GENERATED_ROOT, 'reports', 'registry_proposal.json'), JSON.stringify(registry, null, 2));

console.log(`[apply-manifest] ${rows.length} security proposed docs (applied=0)`);
